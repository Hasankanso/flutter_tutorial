...
import 'package:shopapp/utilities/TimerWidget.dart';
...
class _SMSCodeDialogState extends State<SMSCodeDialog> {
  ...
  final TimerController _timerController = TimerController(duration: Duration(seconds: 120));
    ...
    return AlertDialog(
      ...
      content: Container(
        ...
        child: Column(
          children: [
            ElevatedButton(
                onPressed: () => widget.onSMSCodeEntered(smsCodeController.text, context),
                child: Text("Send")),
            Visibility(
              visible: showResend,
              child: TextButton(onPressed: _resendVerificationCode, child: Text("Resend Code")),
            ),
          ],
        ),
      ),
